        ...
        if (err) {
            ...
            db.all(query, [], (err, rows) => {
                if (err) {
                    ...
                    res.status(200).json(wordMiner(rows[0].text, 300, 100));
                }
            })
        }
    });
 ...
 * Creates a custom handler
 */
const createTopSentiments = (sentiment) => (req, res, next) => {
    db_tool.get((err, db) => {
        if (err) {
            next(err);
        } else {
            ...
            db.all(query, [], (err, rows) => {
                if (err) {
                    ...
                    rows.forEach(row => row.reviews = wordMiner(row.reviews, 1, wordLimit));
                    res.status(200).json({ data: rows, sentiment });
                }
            })
        }
    });
...
exports.topPositiveSentiments = createTopSentiments('positive');
exports.topNegativeSentiments = createTopSentiments('negative');
